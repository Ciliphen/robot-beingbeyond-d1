...
训练 `vertical_grasp_object` 技能用的 YOLO-OBB（有向框）方块检测模型。
产出的 `best.pt` 就是本部署仓 `models/best.pt` 的来源，被技能的 `detector.py::CubeDetector`
...
# beingbeyond_d1_sdk：D1 头部/机械臂 SDK。wheel 随本仓提供，位于
#                     tools/func_verify/lib/，装进运行环境即可。
```

> `capture.py` / `vision.py` 依赖 `pyrealsense2` 和 `beingbeyond_d1_sdk`，只能在真机环境
> 跑；纯训练机上跳过第 0 步、直接从已有图片开始即可。
