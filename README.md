...
(`vertical_grasp_object`)。技能对大模型暴露两个能力,均由 `rbnx chat` 经 pilot 触发:
...
```
robot-beingbeyond-d1/
├── robonix_manifest.yaml        # 部署:系统服务 + d1_arm/d1_hand primitive + vertical_grasp_object skill
...
└── skills/vertical_grasp_object_skill/     # ← 符合 robonix 规范的技能包(社区提交单元)
    ...
    ├── vertical_grasp_object_skill/{__init__.py, main.py, controller.py, primitive_clients.py}
    └── capabilities/
        ├── lib/vertical_grasp_object/srv/{PickCube.srv, PlaceCube.srv}
        ├── pick_cube.v1.toml
        ├── place_cube.v1.toml
        └── driver.v1.toml
```
...
- 本仓自包含:`d1_arm`/`d1_hand` 原语与 soma/urdf 均已随仓提供。向 syswonder 社区
  提交的单元是 `skills/vertical_grasp_object_skill/` 这个技能包(见 robonix package catalog
...
- 硬件抓取高度 `pick_z`、URDF 等在 `robonix_manifest.yaml` 的 skill `config:` 中调整
  (字段见 `skills/vertical_grasp_object_skill/config.spec`)。
