#!/usr/bin/env bash
# Build phase for the vertical_grasp_object skill: run rbnx codegen so the lifecycle
...
#            pick_cube/place_cube toml + lib/vertical_grasp_object/srv/) and emits the
#            `vertical_grasp_object_mcp` module imported by main.py.
set -euo pipefail
...
ENV_PY="${VERTICAL_GRASP_OBJECT_PYTHON:-$HOME/miniconda3/envs/bb_d1_robonix/bin/python3}"
[ -x "$ENV_PY" ] && export PATH="$(dirname "$ENV_PY"):$PATH"

FLAGS=(--mcp)
[[ "${RBNX_BUILD_CLEAN:-}" == "1" ]] && FLAGS+=(--clean)
rbnx codegen -p "$PKG" "${FLAGS[@]}"
echo "[vertical_grasp_object_skill] build done"
