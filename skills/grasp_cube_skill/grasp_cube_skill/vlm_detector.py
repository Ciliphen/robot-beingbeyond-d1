...
target in a head-camera frame and return its normalised bounding box. The box is
then projected to a base-frame (x, y, z) through the *same* hand-eye calibration
...
# axis-aligned box. Mirrors roboarm's user_instruction_prompt; {instruction} is
# substituted at call time. Reply is constrained to a JSON object.
_DETECT_PROMPT = """你是一个智能机械臂的视觉控制中枢。请根据用户的自然语言指令，分析图片场景，\
找出机械臂需要抓取的目标物体，并用边界框标出。确保边界框能完全框住该物体，长条形物体要体现其长度。

坐标系定义：图片左上角为原点 (0, 0)，向右为 x 轴 (0→1)，向下为 y 轴 (0→1)。所有坐标和尺寸都用\
...
- "objects"：数组，每个元素代表一个目标物体，包含：
  ...
  - "box_center_x"：边界框中心 x，0~1，保留 3 位小数。
  - "box_center_y"：边界框中心 y，0~1，保留 3 位小数。
  - "box_width"：边界框宽度，0~1，保留 3 位小数。
  - "box_height"：边界框高度，0~1，保留 3 位小数。

示例（用户指令“抓最右边的蓝色积木”）：
{"failed": false, "objects": [{"class_name": "蓝色方形积木", "box_center_x": 0.821, "box_center_y": 0.131, "box_width": 0.142, "box_height": 0.088}]}
...
class VLMDetector(HeadCameraProjector):
    ...
    def detect(self, instruction: str) -> List[Dict[str, object]]:
        ...
        the result feeds straight into ``pick_cube``. Returns an empty list when
        the VLM reports no matching target. Raises on VLM / parse failure.
        """
        ...
        prompt = _DETECT_PROMPT.replace("{instruction}", instr)
        ...
        for obj in raw_objs:
            ...
            # Axis-aligned box (rotation 0): grasp across the shorter edge.
            grasp_angle = estimate_grasp_angle_deg(u_px, v_px, w_px, h_px, 0.0)
